...
$$ P_{S \sim D^m} ( L_D(A(S),f) \le \varepsilon ) \ge 1 - \delta, $$
where we let $A(S) \in H$ denote the hypothesis picked by the algorithm $A$ when receiving the samples $S = (x_i,f(x_i))_{i \in [1,n]}$.
