...
**All** quasipolynomial algorithms belong to one of three families: separating automata, value iteration, and fixed point.
Each algorithm in these families is parametrised by a concrete universal tree, which dictates its complexity.
</p>
...
The definition, as well as upper and lower bounds on universal trees, can be found in [this post]({{ '/blog/universal_trees' | prepend: site.baseurl }}).
...
is good for small games automata. There are no blog posts on them (yet?).
...
Since the notion of universal graphs applies to much more objectives (said differently: universal trees are universal graphs for parity objectives), it opens many perspectives.
