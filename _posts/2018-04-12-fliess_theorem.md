...
A representation computes a function $$f : A^* \to \R$$ by $$f(w) = \lambda(\mu(w))$$.

One can show that through the choice of a basis for $$V$$ a representation induces a weighted automaton, and conversely.

We consider the vector space $\R^{A^*}$, which has infinite dimension.
We simplify the notation $$H_f(w,\varepsilon)$$ into $H(w)$.
We let $V$ denote the vector space generated by $$\{ H_w \mid w \in A^* \}$$, it has finite dimension since $\rk(H_f)$ is finite.
Define
...
Then we extend $\mu$ into a morphism $$\mu : A^* \to V$$, and let $\lambda(H_w) = f(w)$.
