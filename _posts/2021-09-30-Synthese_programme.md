...
<p class="intro"><span class="dropcap">L</span>a notion de programme est au centre de l'informatique : c'est l'interface entre l'utilisateur et la machine, le moyen par lequel un humain peut communiquer à la machine des opérations à effectuer pour réaliser une tâche de manière automatique.
Les langages de programmation facilitent cette communication : il en existe une très grande variété chacun adapté aux tâches envisagées, par exemple programmation web, réseau, mobiles, etc.
...
Commençons par un petit tour d'horizon des succès récents :
...
Prenons l'exemple de prédictions aux échecs : étant donné le classement [ELO](https://fr.wikipedia.org/wiki/Classement_Elo) de deux joueurs, comment prédire le résultat d'un match entre ces deux joueurs ? Notons $R_A$ et $R_B$ les deux valeurs reflétant le niveau des joueurs, le système ELO prédit que le joueur A gagne avec probabilité $$\frac{10^{R_A / 400}}{10^{R_A / 400} + 10^{R_B / 400}}$$. La synthèse de programmes peut permettre de trouver une telle formule, c'est ce que fait par exemple l'outil académique [PSketch](https://dx.doi.org/10.1145/2737924.2737982) : à partir d'échantillons numériques d'un système réel, il produit une structure de modèle représentant au mieux les observations du système. 
...
En synthèse de programme il est très courant d'utiliser des langages inventés, spécifiques à un domaine d'application ("domain specific language", ou DSL, en anglais), dont les macros d'Excel forment un exemple.
...
Pour décrire ces notions de manière intuitive, on s'appuie sur la figure ci-dessous.
...
Quel est le lien avec les programmes ? Chaque arbre représente un programme. 
Ici c'est abstrait, parce que les symboles (f,g,a,b) n'ont pas de signification. 
L'exemple ci-dessous rend les choses plus concrètes : les symboles sont des primitives du langage de programmation, à savoir take, head, et rev, qui sont des primitives classiques pour manipuler des listes,
et des variables l1 et l2, qui sont les entrées du programme.
...
L'explosion de performances des logiciels de résolution de contraintes, SAT pour les formules booléennes, 
et les SMT (Satisfiability Modulo Theories) pour les théories logiques plus avancées, ont directement impacté les progrès en synthèse de programme. 
...
Elle est décrite par exemple dans cet [article](https://sygus.org/assets/pdf/CACM'18_Search-based_Program_Synthesis.pdf) accessible aux non-spécialistes.
...
Par exemple, [DreamCoder](https://dl.acm.org/doi/10.1145/3453483.3454080) permet au réseau de neurones de "rêver" et ainsi de décrire de nouvelles primitives pour enrichir le DSL.
...
La compétition [SyGuS](https://sygus.org/) est un bon thermomètre des progrès effectués, et une captivante introduction à cette initiative est accessible dans cet [article](https://sygus.org/assets/pdf/CACM'18_Search-based_Program_Synthesis.pdf).
...
Il s'agit d'une longue liste de puzzles, présentés sous la forme de grilles à compléter.
N'importe quel enfant peut résoudre la plupart de ces puzzles, même s'il ne sait pas lire, et quelque soit son environnement culturel ou social.
Par contre, aucun logiciel existant n'est capable de résoudre ne serait-ce que la moitié de ces grilles !

## Références

En plus des références donnés dans l'article, les [notes de cours](https://people.csail.mit.edu/asolar/SynthesisCourse/index.htm) d'Armando Solar-Lezama du MIT sont une mine d'informations presque inépuisable.
